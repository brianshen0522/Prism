...
async function getChartBucketMinutes(): Promise<number> {
  const s = await prism.systemSetting.findUnique({ where: { key: 'dashboard_chart_bucket_minutes' } });
  const v = parseInt(s?.value ?? '', 10);
...
export async function dashboardRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/dashboard/chart', { preHandler: privileged }, async (_req, reply) => {
    ...
    const [hours, bucketMinutes] = await Promise.all([getChartHours(), getChartBucketMinutes()]);
    const bucketSeconds = bucketMinutes * 60;

    const rows = await prism.$queryRaw<Row[]>(Prisma.sql`
      SELECT
        to_timestamp(floor(extract(epoch from req_timestamp) / ${bucketSeconds}) * ${bucketSeconds}) AS bucket,
